...
fn test_partitioned_not_over_empty_universe_does_not_fire() {
    // An asset whose partition universe is currently empty (e.g. a dynamic
    // partitioned asset before any keys exist) must not report fired for a
    // Not(...) clause: complement of Empty over an empty universe is Empty.
    let record = make_materialized_record("a", 100);
    let records = HashMap::from([("a".into(), record.clone())]);
    let deps = HashMap::new();
...
async fn test_pending_eviction_untracks_all_assets_of_a_multi_asset_run() {
    // A single run_id can materialize several assets (a joint run). When it
    // goes phantom, eviction must untrack EVERY asset it covered, not just the
    // last one registered — otherwise the others stay in_progress forever.
    let (storage, mut cache) = pending_test_setup().await;
