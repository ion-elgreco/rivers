...
fn _core(m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    m.add_class::<retry::PyRetryPolicy>()?;
    m.add_class::<retry::PyBackoff>()?;
    m.add_class::<retry::PyRetryOn>()?;
    m.add_class::<retry::PyComputeEscalation>()?;
    m.add_class::<retry::PyFailureReason>()?;
