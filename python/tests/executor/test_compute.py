...
def test_compute_on_multi_asset(storage):
    """Compute is declared on the multi-asset itself — one step, one pod."""

    @rs.Asset.from_multi(
        output_defs=[rs.AssetDef("cd_a"), rs.AssetDef("cd_b")],
        compute=rs.Compute(memory="2Gi"),
    )
    def multi_sized():
...
def test_asset_def_rejects_compute():
    """Per-output compute is not a thing — it lives on ``from_multi``."""
