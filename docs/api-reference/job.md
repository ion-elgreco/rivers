...
| `retry` | `RetryPolicy \| str \| None` | `None` | Job-level retry default — a [`RetryPolicy`](retries.md) or a `retries` registry name. Assets with their own policy keep it. Applies to asset steps only; task steps trigger a `UserWarning` at `resolve()`. |
