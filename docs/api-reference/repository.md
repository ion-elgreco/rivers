...
| `retries` | `dict[str, RetryPolicy] \| None` | `None` | Named retry policies referenced by `retry="name"` on assets, tasks, jobs, and `materialize()`. See [Retries & Compute](retries.md). |
| `default_retry_policy` | `RetryPolicy \| str \| None` | `None` | Repo-wide retry default (lowest precedence: asset/task > job > this) — a policy or a `retries` registry name. |
...
| `retry` | `RetryPolicy \| str \| None` | `None` | Run-level retry default — a [`RetryPolicy`](retries.md) or a `retries` registry name. Nodes with their own policy keep it. |
